...
assert.match(index, /id="zzr-wallet-menu"/);
assert.match(index, /id="zzr-wallet-garage"/);
...
assert.match(index, /🐱<\/span> ZıpZıp Taner/);
assert.match(index, /🚗<\/span> Yağmur/);
assert.match(index, /data-color="6" aria-label="Kırmızı"/);
assert.match(index, /data-accessory="cap"/);
assert.match(index, /data-accessory="crown"/);
assert.match(index, /data-accessory="pirate"/);
...
assert.match(index, /item\.type === "pit"/);
assert.match(index, /\[0\.31, 0\.68, 0\.91\]/);
assert.match(index, /function drawPit\(segment\)/);
assert.match(index, /function fallIntoPit\(\)/);
assert.match(index, /SEGMENT_LENGTH \* 18/);
assert.match(index, /game\.pitFalls \+= 1/);
assert.match(index, /size \* 1\.84/);
...
assert.match(index, /const ACCESSORIES =/);
assert.match(index, /function loadEconomy\(\)/);
assert.match(index, /function creditMoney\(amount\)/);
assert.match(index, /function drawAccessory\(context, dims, accessoryId\)/);
...
assert.match(worker, /CACHE_NAME = "zipzip-ralli-v6-pits-economy"/);
...
const accessoryIds = [...index.matchAll(/data-accessory="([^"]+)"/g)].map((match) => match[1]);
assert.equal(accessoryIds.length, 13, "Sade seçim ve 12 satın alınabilir aksesuar bulunmalı");
